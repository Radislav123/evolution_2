...
class Settings(Singleton):
    def __init__(self) -> None:
        if not self.inited:
            ...
            self.CELL_SIZE = 32
            self.CELL_SHAPE = self.decompose(self.CELL_SIZE, 3)
            self.CHUNK_COUNT = 1
    ...
    @staticmethod
    def decompose(value: int, components: int) -> Vec3:
        result = [1] * components
        dividers = []
        divider = 2
        while divider <= value:
        ...
        for divider in dividers[::-1]:
            prediction = [x * divider for x in result]
            result[prediction.index(min(prediction))] *= divider
    ...
    def check(self) -> None:
        ...
        if 0 > self.CELL_SIZE or 63 < self.CELL_SIZE:
            raise SettingError(f"self.CELL_SIZE ({self.CELL_SIZE}) must be in [1; 63]")
